...
class AlmanacToLTLf(Transformer):
    def formula(self, items):
        ...
        if name == "reactive_response":
            ...
            if n == 0:
                return f"G({in_atom} -> (!last U {out_atom}))"
        ...
        if name == "proactive_alternate_precedence":
            if len(args) != 2:
                raise ValueError("proactive_alternate_precedence expects 2 arguments")
            out_a, out_b = args
            return f"((!{out_b} U {out_a}) | G(!{out_b})) & G({out_b} -> X((!{out_b} U {out_a}) | G(!{out_b})))"

        if name == "proactive_cooldown":
            if len(args) == 1:
                out_atom, = args
                return f"G({out_atom} -> X(!{out_atom} U (last | !{out_atom})))"
            if len(args) == 2:
                out_atom, n = args
                if not isinstance(n, int) or n < 1:
                    raise ValueError("proactive_cooldown expects second argument to be integer >= 1")
                nexts = []
                for i in range(1, n + 1):
                    nexts.append(f"{('X(' * i)}(!{out_atom} | last){')' * i}")
                inner = " & ".join(nexts)
                return f"G({out_atom} -> ({inner}))"
            raise ValueError("proactive_cooldown expects 1 or 2 arguments")

        if name == "proactive_alternate_response":
            if len(args) != 2:
                raise ValueError("proactive_alternate_response expects 2 arguments")
            out_a, out_b = args
            return f"G({out_a} -> X(!{out_a} U ({out_b} | last)))"
...
ALMANAC_TEMPLATES = {
    ...
    "proactive_alternate",
    "proactive_alternate_precedence",
    "proactive_cooldown",
    "proactive_alternate_response",
    "initial_state",
}
