...
if __name__ == "__main__":
    controller = LTLfController(config_file="case/semaphore.txt", aggressive_pruning=False)
    ...
    with open("out/dot/semaphore_mealy_graph.dot", "w") as f_mealy:
        f_mealy.write(dot_mealy)
    convert_dot_to_png(dot_mealy, "out/png/semaphore_mealy_graph.png")
    ...
    with open("out/step_actions.txt", "w") as f_actions:
        ...
        for i in range(10):
            ...
            sensor_dict = {
                "ew_det": random.choice([True, False]),
                "ns_det": random.choice([True, False])
            }
